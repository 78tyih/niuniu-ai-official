  ...
  return (
    <div>
      <header className="flex flex-col gap-3 border-b border-[#edf0f3] pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div><div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#ff6a1a]">Subscription ledger</div><h1 className="mt-2 text-2xl font-bold tracking-tight">订阅与账户权益</h1><p className="mt-2 text-sm text-[#7d8797]">集中查看订阅订单、授权交付和套餐附带的使用权益记录。</p></div>
      </header>
      ...
      <section className="mt-6 grid gap-px overflow-hidden rounded-xl border border-[#e5e8ed] bg-[#e5e8ed] sm:grid-cols-[0.85fr_1.15fr]">
        <div className="bg-[#14171f] p-5 text-white"><div className="flex items-center gap-2 text-xs text-white/55"><Wallet size={15} /> 套餐使用权益</div><div className="mt-3 text-3xl font-bold tabular-nums">{loading ? '—' : credits.toLocaleString('zh-CN')}</div><p className="mt-2 text-xs leading-relaxed text-white/50">权益由有效订阅套餐发放，仅用于 AI 服务消耗，不提供单独购买或充值入口。</p></div>
        <div className="bg-white p-5"><div className="flex items-center gap-2 text-xs font-medium text-[#7d8797]"><CreditCard size={15} /> 订阅订单与授权</div><p className="mt-3 text-sm leading-relaxed text-[#4f5968]">已支付订单会在此展示订阅套餐、支付状态和授权交付信息。3 天体验卡每个账户只能购买一次。</p></div>
      </section>
